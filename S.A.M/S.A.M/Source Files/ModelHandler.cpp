...
bool ModelHandler::LoadOBJData2(string OBJFileName, string colourFileName, ID3D11Device* device, ID3D11DeviceContext* deviceContext)
{
	bool _ifOK;
...
bool ModelHandler::CreateBuffers(ID3D11Device* device)
{
	...
	m_vertexInput = new VertexInputType[m_faceCount * 3];

	for (int i = 0; i < (m_faceCount * 3); i += 3)
	{
		m_vertexInput[i].position = XMFLOAT3(m_vertices[m_faces[i / 3].vIndex1 - 1].x, m_vertices[m_faces[i / 3].vIndex1 - 1].y, m_vertices[m_faces[i / 3].vIndex1 - 1].z);
		m_vertexInput[i].uv = XMFLOAT2(m_texcoords[m_faces[i / 3].tIndex1 - 1].x, m_texcoords[m_faces[i / 3].tIndex1 - 1].y);
		m_vertexInput[i].normal = XMFLOAT3(m_normals[m_faces[i / 3].nIndex1 - 1].x, m_normals[m_faces[i / 3].nIndex1 - 1].y, m_normals[m_faces[i / 3].nIndex1 - 1].z);

		m_vertexInput[i + 1].position = XMFLOAT3(m_vertices[m_faces[i / 3].vIndex2 - 1].x, m_vertices[m_faces[i / 3].vIndex2 - 1].y, m_vertices[m_faces[i / 3].vIndex2 - 1].z);
		m_vertexInput[i + 1].uv = XMFLOAT2(m_texcoords[m_faces[i / 3].tIndex2 - 1].x, m_texcoords[m_faces[i / 3].tIndex2 - 1].y);
		m_vertexInput[i + 1].normal = XMFLOAT3(m_normals[m_faces[i / 3].nIndex2 - 1].x, m_normals[m_faces[i / 3].nIndex2 - 1].y, m_normals[m_faces[i / 3].nIndex2 - 1].z);

		m_vertexInput[i + 2].position = XMFLOAT3(m_vertices[m_faces[i / 3].vIndex3 - 1].x, m_vertices[m_faces[i / 3].vIndex3 - 1].y, m_vertices[m_faces[i / 3].vIndex3 - 1].z);
		m_vertexInput[i + 2].uv = XMFLOAT2(m_texcoords[m_faces[i / 3].tIndex3 - 1].x, m_texcoords[m_faces[i / 3].tIndex3 - 1].y);
		m_vertexInput[i + 2].normal = XMFLOAT3(m_normals[m_faces[i / 3].nIndex3 - 1].x, m_normals[m_faces[i / 3].nIndex3 - 1].y, m_normals[m_faces[i / 3].nIndex3 - 1].z);
	}
...
bool ModelHandler::CreateBuffers2(ID3D11Device* device)
{
	D3D11_BUFFER_DESC _OBJvertexBufferDesc, _OBJColDesc;
	D3D11_SUBRESOURCE_DATA _OBJvertexData, _OBJColourData;
	OBJColourType* _OBJColour;
