...
OptionsMenu::OptionsMenu(ID3D11Device* Device, ID3D11DeviceContext* DeviceContext, int ScreenHeight, int ScreenWidth, Input* input) : Screen(Device, DeviceContext, ScreenHeight, ScreenWidth, input)
{
	...
	m_choices[0].m_origin = m_font->MeasureString(m_option1.c_str()) / 2.f;
	m_choices[0].m_position.x = m_choices[0].m_origin.x;
	m_choices[0].m_color = Colors::Crimson;

	m_choices[1].m_position.y = m_screenHeight / 2;
	m_choices[1].m_origin = m_font->MeasureString(m_option2.c_str()) / 2.f;
	m_choices[1].m_position.x = m_choices[1].m_origin.x;
	m_choices[1].m_color = Colors::Crimson;

	m_choices[2].m_position.y = m_screenHeight * 6 /7;
	m_choices[2].m_origin = m_font->MeasureString(m_menu.c_str()) / 2.f;
	m_choices[2].m_position.x = m_screenWidth / 2.f;
...
void OptionsMenu::Render()
{
	...
	m_font->DrawString(m_spriteBatch.get(), m_menu.c_str(), m_choices[2].m_position, m_choices[2].m_color, 0.f, m_choices[2].m_origin);

	m_font->DrawString(m_spriteBatch.get(), m_option1.c_str(), m_choices[1].m_position, m_choices[1].m_color, 0.f, m_choices[1].m_origin);

	m_font->DrawString(m_spriteBatch.get(), m_option2.c_str(), m_choices[0].m_position, m_choices[0].m_color, 0.f, m_choices[0].m_origin);
